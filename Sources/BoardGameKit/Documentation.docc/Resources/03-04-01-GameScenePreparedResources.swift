...
final class CrazyEightsGameScene: SCNNode, GameScene3D {
    ...
    private var activePlayerSpotlights: ActivePlayerSpotlights3D!
    private let chooseRankSuitDialog = ChooseRankSuitDialog()
    ...
    func load(localizedPlayers: [LocalizedPlayer], options: CrazyEightsOptions) async throws {
        try await PreparedResources.load([.init(paths: ["Art.scnassets"], options: .loadMaterials)])
        ...
        activePlayerSpotlights = ActivePlayerSpotlights3D(players: players, lights: [ActivePlayerSpotlights3D.createSpotLight()])
        addChildNode(activePlayerSpotlights)
        
        eventHandler.register(startRound)
        eventHandler.register(playCard)
        eventHandler.register(chooseRankSuit)
    ...
    func handle(eventGroup: GameManager.EventGroupResponse) throws {
        activePlayerSpotlights.update(states: eventGroup.playerStateChanges)
    ...
    func enableActions(_ enable: Bool, for player: CrazyEightsPlayerNode) {
        for state in eventHandler.state(for: player) {
            ...
            case .chooseRankSuit:
                overlay?.showChild(chooseRankSuitDialog, enable)
            }
        }
    }
    ...
    private func playCard(data: FailableGameEventResult<CrazyEightsGame.PlayCardEvent>) {
        ...
        case .success(let data):
            ...
            addTimeOffset(1)
            if data.chooseRankSuit == true {
                chooseRankSuitDialog.state = .init(choiceAction: { [unowned self] choice in
                    actionHandler?.sendAction(CrazyEightsGame.ChooseRankSuitAction(rankSuit: choice))
                })
            }
    ...
    private func chooseRankSuit(data: CrazyEightsGame.ChooseRankSuitEvent) {
        let title = switch data.rankSuit {
        case .rank(let rank):
            rank.formatted()
        case .suit(let suit):
            suit.rawValue
        }
        let node = TextNode3D(string: title, fontSize: 30, alignment: .center)
        node.simdPosition = table.playedCards.simdPosition + Vector3(x: 0, y: 0.05, z: 0)
        addAnimation(TimelineAnimation(id: "chooseRankSuit", subject: node, startTime: GameSceneContext.current.timeline.currentTime + GameSceneContext.current.timeline.timeOffset, duration: 3, enter: { [self] _ in
            addChildNode(node)
        }, progress: { t in
            node.opacity = t < 0.3 ? t / 0.3 : t < 0.7 ? 1 : (1 - t) / 0.3
        }, exit: { _ in
    ...
    private func endRound(data: CrazyEightsGame.EndRoundEvent) {
        for (player, points) in data.points {
            let player = players.first(id: player)!
            player.points.setText(points.formatted(), duration: 2)
        }
        addTimeOffset(2)
