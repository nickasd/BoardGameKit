...
struct CrazyEightsConfiguration: Configuration {
    ...
    let client = ClientConfiguration(scenes: .init(game: .scene3D(CrazyEightsGameScene.self), rules: CrazyEightsRulesView.self), sampleGamesPath: "Sample games")
...
extension CrazyEightsStrategy: LocalizedLabel {
    
    var localizedLabel: String {
        return switch self {
        case .dummy:
            NSLocalizedString("strategy.dummy", value: "Dummy", comment: "")
        }
    }
